                ...
                <v-col cols="7" lg="8" md="9" sm="8" xs="8">
                  <v-row class="ma-0">
                    ...
                    <v-col
                      ...
                      sm="6"
                      xs="6"
                      class="d-flex justify-end order-2 order-lg-4 order-md-4"
                    >
                      <div>
                        <!-- <v-btn fab color="#ffffff" small text depressed -->
                        <v-icon color="#bdbdbd" @click="hapuskeranjang(dataa)"
                          >mdi-delete</v-icon
                        >
                        <!-- </v-btn> -->
                      </div>
  ...
  methods: {
    getkeranjang() {
      axios
        ...
        .then((respon) => {
          ...
          this.totalprice.harga = respon.data?.user_keranjang[0].harga;
        });
    },
    countplus(dataa) {
      if (dataa.kuantitas < dataa.stok) {
        dataa.kuantitas++;
      } else {
      }
      this.totalprice.harga = parseInt(dataa.kuantitas) * parseInt(dataa.harga);
    ...
    countmin(dataa) {
      ...
      this.totalprice.harga =
        parseInt(this.totalprice.harga) - parseInt(dataa.harga);
    },
    hapuskeranjang(item) {
    ...
    nomer() {
      ...
      let pesan =
        ...
        this.datakeranjang.user_keranjang[0].nama_barang +
        " dengan%20varian%20" +
        this.datakeranjang.user_keranjang[0].nama_varian +
        "%20seharga%20Rp." +
        this.totalprice?.harga;
      window.open(
    ...
    transaksi() {
      this.datatransakasipost.user_id = this.userid;
      this.datatransakasipost.toko_id =
        this.datakeranjang.user_keranjang[0].toko_id;
      this.datatransakasipost.barang_id =
        this.datakeranjang.user_keranjang[0].barang_id;
      this.datatransakasipost.varian_id =
        this.datakeranjang.user_keranjang[0].varian_id;
      this.datatransakasipost.kategori_id =
        this.datakeranjang.user_keranjang[0].kategori_id;
      this.datatransakasipost.jumlah = this.kuan;
      this.datatransakasipost.total_harga = this.totalprice?.harga;
      console.log(this.datatransakasipost);
      axios
        .post(
          "http://localhost:8000/api/createtransaksi",
          this.datatransakasipost
        )
        .then((respon) => {
